...
def writePPSSPPControls(system):
    iniControls = configparser.ConfigParser(interpolation=None)
    # To prevent ConfigParser from converting to lower case
    iniControls.optionxform = str
    if os.path.exists(ppssppControls):
        try:
            with io.open(ppssppControls, 'r', encoding='utf_8_sig') as fp:
                iniControls.readfp(fp)
        except:
            pass

    createPPSSPPControls(iniControls, system)
    # Save the ini file
    if not os.path.exists(os.path.dirname(ppssppControls)):
        os.makedirs(os.path.dirname(ppssppControls))
    with open(ppssppControls, 'w') as controlsfile:
        iniControls.write(controlsfile)

def createPPSSPPControls(iniControls, system):
    if not iniControls.has_section("ControlMapping"):
        iniControls.add_section("ControlMapping")

    # controls are called via evmapy. configuring specific hotkey in ppsspp is not simple without patching
    iniControls.set("ControlMapping", "Rewind",        "1-131")
    iniControls.set("ControlMapping", "Fast-forward",  "1-132")
    iniControls.set("ControlMapping", "Save State",    "1-133")
    iniControls.set("ControlMapping", "Load State",    "1-134")
    iniControls.set("ControlMapping", "Previous Slot", "1-135")
    iniControls.set("ControlMapping", "Next Slot",     "1-136")
    iniControls.set("ControlMapping", "Screenshot",    "1-137")
