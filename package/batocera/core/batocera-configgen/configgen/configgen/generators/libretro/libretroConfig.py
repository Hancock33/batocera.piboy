...
systemToRetroachievements = {'amstradcpc', 'atari2600', 'arduboy', 'atari7800', 'jaguar', 'channelf', 'colecovision', 'dreamcast', 'atomiswave', 'naomi', 'nes', 'snes', 'virtualboy', 'n64', 'sg1000', 'mastersystem', 'megadrive', 'segacd', 'sega32x', 'saturn', 'pcengine', 'pcenginecd', 'supergrafx', 'psx', 'mame', 'fbneo', 'neogeo', 'lightgun', 'apple2', 'lynx', 'wswan', 'wswanc', 'gb', 'gbc', 'gba', 'sgb', 'nds', 'pokemini', 'gamegear', 'ngp', 'ngpc', 'supervision', 'sufami', 'pc88', 'pcfx', '3do', 'intellivision', 'o2em', 'vectrex', 'wonderswan', 'psp', 'snes-msu1', 'satellaview', 'wasm4'};
...
coreToRetroachievements = {'beetle-saturn', 'blastem', 'bluemsx', 'bsnes', 'bsnes_hd', 'cap32', 'desmume', 'duckstation', 'fbneo', 'fceumm', 'flycast', 'flycast2021', 'freechaf', 'freeintv', 'gambatte', 'genesisplusgx', 'genesisplusgx-wide', 'handy', 'kronos', 'mednafen_lynx', 'mednafen_ngp', 'mednafen_psx', 'mednafen_supergrafx', 'mednafen_wswan', 'melonds', 'mesen', 'mesens', 'mgba', 'mupen64plus-next', 'o2em', 'opera', 'parallel_n64', 'pce', 'pce_fast', 'pcfx', 'pcsx_rearmed', 'picodrive', 'pokemini', 'potator', 'ppsspp', 'prosystem', 'quasi88', 'snes9x', 'snes9x_next', 'stella', 'stella2014', 'swanstation', 'vb', 'vba-m', 'vecx', 'virtualjaguar'}
...
def createLibretroConfig(generator, system, controllers, guns, rom, bezel, shaderBezel, gameResolution, gfxBackend):
    ...
    if (system.isOptSet("input_joypad_driver")):
        retroarchConfig['input_joypad_driver'] = system.config['input_joypad_driver']

    retroarchConfig['input_driver'] = 'udev'                    # driver for mouse/keyboard. udev required for guns.
    if (system.isOptSet("input_driver")):
        retroarchConfig['input_driver'] = system.config['input_driver']
    ...
    if system.config['core'] == 'puae2021':
        if system.name != 'amigacd32':
            if system.isOptSet('controller1_puae2021'):
                ...
                retroarchConfig['input_libretro_device_p1'] = '1'
            if system.isOptSet('controller2_puae2021'):
                retroarchConfig['input_libretro_device_p2'] = system.config['controller2_puae2021']
            else:
                retroarchConfig['input_libretro_device_p2'] = '1'
        else:
            retroarchConfig['input_libretro_device_p1'] = '517'     # CD 32 Pad

    ## AMIGA OCS-ECS/AGA/CD32 (uae4arm)
    if system.config['core'] == 'uae4arm':
        if system.isOptSet('controller1_uae4arm'):
            retroarchConfig['input_libretro_device_p1'] = system.config['controller1_uae4arm']
        else:
            retroarchConfig['input_libretro_device_p1'] = '1'
        if system.isOptSet('controller2_uae4arm'):
