...
WINE_TKG_DEPENDENCIES = host-bison host-flex host-wine-custom
HOST_WINE_TKG_DEPENDENCIES = host-bison host-flex

define WINE_TKG_AUTOGEN
	# Create folder for install
	mkdir -p $(TARGET_DIR)/usr/wine/tkg
...
WINE_TKG_PRE_CONFIGURE_HOOKS += WINE_TKG_AUTOGEN
HOST_WINE_TKG_PRE_CONFIGURE_HOOKS += WINE_TKG_AUTOGEN

# Wine needs its own directory structure and tools for cross compiling
WINE_TKG_CONF_OPTS = LDFLAGS="-Wl,--no-as-needed -lm" CPPFLAGS="-DMPG123_NO_LARGENAME=1" \
...
define WINE_TKG_REMOVE_INCLUDES_HOOK
	rm -Rf $(TARGET_DIR)/usr/wine/tkg/include
endef
