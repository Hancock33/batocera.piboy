...
_SOURCE_DIR: Final = Path('/usr/bin32/linuxloader')
...
class LinuxLoader(ControllersMixin, Emulator):
    ...
    async def configure(self) -> Command:
        ...
        environment: dict[str, str | Path] = {
            # Libraries
            'LD_LIBRARY_PATH': f'/lib32:/usr/bin32/linuxloader/extralibs:/lib:/usr/lib:{_SOURCE_DIR}:{rom_dir}',
    ...
    def _build_conf_file(self, conf: Configuration, rom_dir: Path, rom_name: str) -> None:
        ...
        conf.set(
            'P1_CROSSHAIR_PATH',
            '/usr/bin32/linuxloader/crosshairs/p1_crosshair.png' if crosshairs else '',
        )
        conf.set(
            'P2_CROSSHAIR_PATH',
            '/usr/bin32/linuxloader/crosshairs/p2_crosshair.png' if crosshairs else '',
        )
    ...
    def _setup_libraries(self, rom_dir: Path, rom_name: str) -> None:
        ...
        if any(keyword in rom_name.lower() for keyword in ('harley', 'hdkotr', 'spicy', 'rambo', 'hotdex', 'dead ex')):
            ...
            src_cg = Path('/usr/bin32/linuxloader/extralibs/libCg.so.harley')
            src_cg_gl = Path('/usr/bin32/linuxloader/extralibs/libCgGL.so.harley')
        ...
        # Scan first; if diff, overwrite
            ...
            dest_cg = Path(rom_dir) / 'libCg.so'
            dest_cg_gl = Path(rom_dir) / 'libCgGL.so'
            src_cg = Path('/usr/bin32/linuxloader/extralibs/libCg.so.other')
            src_cg_gl = Path('/usr/bin32/linuxloader/extralibs/libCgGL.so.other')
            if src_cg.exists() and (not dest_cg.exists() or not filecmp.cmp(src_cg, dest_cg, shallow=False)):
                shutil.copy2(src_cg, dest_cg)
