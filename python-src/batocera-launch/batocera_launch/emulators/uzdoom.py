...
from batocera_common.paths import BATOCERA_SHARE_DIR, CONFIGS, LOGS, SAVES
...
class UZDoom(Emulator):
    ...
    def _determine_api_config(self) -> str:
        uzdoom_api = self.config.get_str('uz_api', '0')
        arch_path = BATOCERA_SHARE_DIR / 'batocera.arch'

        # "OpenGL" (0) means the native OpenGLES backend (2) on non-x86_64,
        # where that's the only backend actually built (see HAVE_GLES2 in
        # uzdoom.mk); real desktop OpenGL only exists on x86_64.
        if uzdoom_api == '0' and arch_path.exists():
            arch = arch_path.read_text().strip()
            if arch != 'x86_64':
                uzdoom_api = '2'

        return f'vid_preferbackend {uzdoom_api}\n'

    def _create_script_file(self, script_file: Path, api_config: str) -> None:
        content = (
            '# This file is automatically generated by batocera-launch\n'
            f'logfile "{LOGS / "uzdoom.log"}"\n'
    ...
    def _update_ini_file(self, ini_file: Path, sound_fonts_dir: Path, fm_banks_dir: Path) -> None:
        ...
        # Vsync
        set_uz_vsync = self.config.get_bool('uz_vsync', return_values=('true', 'false'))
        ini.set_value('[GlobalSettings]', 'vid_vsync', set_uz_vsync)
        # Set joystick option
        ...
        # UZDoom's internal joystick handler for controllers is not dynamic.
    ...
    async def configure(self) -> Command:
        ...
        script_file = self.config_dir / 'uzdoom.cfg'
        ini_file = self.config_dir / 'uzdoom.ini'
