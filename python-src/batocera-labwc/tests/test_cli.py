...
@pytest.fixture
def rules_dir(fs: FakeFilesystem) -> Path:
    path = BATOCERA_SHARE_DIR / 'labwc'
    path.mkdir(parents=True, exist_ok=True)

    for key, contents in _RULE_SETS.items():
        (path / f'{key}.labwc-rules.yml').write_text(contents)

    return path


@pytest.fixture
def run_main(mocker: MockerFixture) -> Callable[..., None]:
    def _run(*argv: str) -> None:
        mocker.patch('sys.argv', ['batocera-labwc-config', *argv])
        main()
...
class TestMain:
    def test_reconfigure_only(
        self,
        run_main: Callable[..., None],
        mocker: MockerFixture,
    ) -> None:
        LabWCConfig = mocker.patch('batocera_labwc.cli.LabWCConfig')

        run_main('--reconfigure')

        LabWCConfig.reconfigure.assert_called_once_with()
        LabWCConfig.assert_not_called()

    def test_applies_global_rules_by_default(
        self,
        rc_path: Path,
        rules_dir: Path,
        run_main: Callable[..., None],
    ...
    def test_empty_rules_yaml_applies_no_window_rules(
        ...
        path = BATOCERA_SHARE_DIR / 'labwc' / '_global.labwc-rules.yml'
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text('')
    ...
    def test_save_only_when_no_outputs_or_touchscreen(
        ...
        run_main('--config-path', str(rc_path))
