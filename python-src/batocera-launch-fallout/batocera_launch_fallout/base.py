...
class FalloutBase(Emulator):
    ...
    def ini_file_path(self) -> Path:
        return self.config_dir / self.INI_FILE_NAME
    ...
    async def configure(self) -> Command:
        ...
        if not self.config_file_path.exists() and self.src_config_file_path.exists():
            shutil.copy(self.src_config_file_path, self.config_file_path)

        # Now copy the ini file to the config directory
        if not self.ini_file_path.exists() and self.src_ini_file_path.exists():
            shutil.copy(self.src_ini_file_path, self.ini_file_path)
        ...
        with self.ini_file_path.open('w') as configfile:
